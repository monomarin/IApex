 ...
 */
import { pluginManifestV1Schema } from "@iapexai/shared";
import type { IApexPluginManifestV1 } from "@iapexai/shared";
import { PLUGIN_API_VERSION } from "@iapexai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): IApexPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as IApexPluginManifestV1,
        };
      }
