 ...
 *    (`~/.IApex/plugins/`) and `node_modules` for packages matching
 *    the `IApex-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published IApex plugins.
 * Packages matching this pattern are considered IApex plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "IApex-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: IApexPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.IApex/plugins/ local directory
  | "npm"               // npm packages matching IApex-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: IApexPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the IApex-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "IApex-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: IApexPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover IApex plugins installed as npm packages in the current
   * Node.js environment matching the "IApex-plugin-*" naming convention.
   *
   ...
   * the "IApexPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a IApex plugin.
   * Throws if the package is a IApex plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<IApexPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: IApexPluginManifestV1;
    newManifest: IApexPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the IApex plugin naming convention.
 * Accepts both the "IApex-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @iapexai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "IApexPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const IApexPlugin = pkgJson["IApexPlugin"];
  if (
    IApexPlugin !== null &&
    typeof IApexPlugin === "object" &&
    !Array.isArray(IApexPlugin)
  ) {
    const manifestRelPath = (IApexPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: IApexPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumIApexVersion;
}
 ...
 *   packageName: "IApex-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: IApexPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as IApexPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a IApex plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a IApex plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasIApexPlugin = "IApexPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasIApexPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<IApexPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: IApexPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @iapexai/shared exports). Run those workers through
