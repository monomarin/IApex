...
import type { BillingType } from "@iapexai/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_IApexWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__IApex_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    const runtimeConfig = {
      ...resolvedConfig,
      IApexRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.IApexWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.IApexRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.IApexRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.IApexSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.IApexSessionRotationReason = sessionCompaction.reason;
      context.IApexPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.IApexSessionHandoffMarkdown;
      delete context.IApexSessionRotationReason;
      delete context.IApexPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.IApexRuntimeServices = runtimeServices;
        context.IApexRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[IApex] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected IApex_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.IApexRuntimeServices = combinedRuntimeServices;
        context.IApexRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[IApex] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
