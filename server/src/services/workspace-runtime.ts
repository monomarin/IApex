...
import type { AdapterRuntimeServiceReport } from "@iapexai/adapter-utils";
import type { Db } from "@iapexai/db";
import { workspaceRuntimeServices } from "@iapexai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.IApex_WORKSPACE_CWD = input.worktreePath;
  env.IApex_WORKSPACE_PATH = input.worktreePath;
  env.IApex_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.IApex_WORKSPACE_BRANCH = input.branchName;
  env.IApex_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.IApex_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.IApex_WORKSPACE_SOURCE = input.base.source;
  env.IApex_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.IApex_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.IApex_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.IApex_PROJECT_ID = input.base.projectId ?? "";
  env.IApex_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.IApex_AGENT_ID = input.agent.id;
  env.IApex_AGENT_NAME = input.agent.name;
  env.IApex_COMPANY_ID = input.agent.companyId;
  env.IApex_ISSUE_ID = input.issue?.id ?? "";
  env.IApex_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.IApex_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.IApex_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.IApex_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.IApex_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.IApex_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.IApex_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.IApex_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.IApex_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.IApex_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.IApex_PROJECT_ID = input.workspace.projectId ?? "";
  env.IApex_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.IApex_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".IApex", "worktrees");
