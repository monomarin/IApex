...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@iapexai/db";
 ...
 *
 * This implementation delegates to the core IApex domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@iapexai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
