...
import type { IApexPluginManifestV1 } from "@iapexai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __IApex_exports: module.exports,
      __IApex_module: module,
      __IApex_require: requireInSandbox,
      __IApex_filename: realPath,
      __IApex_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__IApex_exports, __IApex_module, __IApex_require, __IApex_filename, __IApex_dirname)`;
