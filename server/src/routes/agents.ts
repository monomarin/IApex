...
import { agents as agentsTable, companies, heartbeatRuns } from "@iapexai/db";
...
import { runClaudeLogin } from "@iapexai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@iapexai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@iapexai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@iapexai/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@iapexai/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.IApex_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeIApexSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readIApexSkillSyncPreference(
