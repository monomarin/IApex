...
import type { Db } from "@iapexai/db";
import {
  agentApiKeys,
  authUsers,
  invites,
...
import type { DeploymentExposure, DeploymentMode } from "@iapexai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "IApex" &&
    normalized !== "IApex-create-agent" &&
    normalized !== "IApex-create-plugin" &&
...
/** Resolve the IApex repo skills directory (built-in / managed skills). */
function resolveIApexSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const IApexSkillsDir = resolveIApexSkillsDir();

  // Build set of IApex-managed skill names
  const IApexSkillNames = new Set<string>();
  if (IApexSkillsDir) {
    try {
      for (const entry of fs.readdirSync(IApexSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) IApexSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isIApexManaged: IApexSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.IApexApiUrl)) {
    const legacyIApexApiUrl = nonEmptyTrimmedString(input.IApexApiUrl);
    if (legacyIApexApiUrl) merged.IApexApiUrl = legacyIApexApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    IApexApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.IApexApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawIApexApiUrl =
    typeof defaults.IApexApiUrl === "string"
      ? defaults.IApexApiUrl.trim()
      : "";
  if (rawIApexApiUrl) {
    try {
      const parsedIApexApiUrl = new URL(rawIApexApiUrl);
      if (
        parsedIApexApiUrl.protocol !== "http:" &&
        parsedIApexApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_IApex_api_url_protocol",
          level: "warn",
          message: `IApexApiUrl must use http:// or https:// (got ${parsedIApexApiUrl.protocol}).`
        });
      } else {
        normalized.IApexApiUrl = parsedIApexApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_IApex_api_url_configured",
          level: "info",
          message: `IApexApiUrl set to ${parsedIApexApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_IApex_api_url_invalid",
        level: "warn",
        message: `Invalid IApexApiUrl: ${rawIApexApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your IApex host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "IApex is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm IApexai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/IApex";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/IApex-claimed-api-key.json and load IApex_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: IApexApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the IApex hostname is reachable and allowed via `pnpm IApexai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this IApex API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/IApex/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # IApex OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          IApexApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, IApex generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "IApexApiUrl": "https://IApex-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in IApex before key claim is allowed.
    ...
    - ~/.openclaw/workspace/IApex-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/IApex-claimed-api-key.json

    And set the IApex_API_KEY and IApex_API_URL in your environment variables as specified here:
    ...
    Then set IApex_API_KEY and IApex_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install IApex skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your IApex_API_URL to the top of your skill and note the path to your IApex_API_URL
    ...
    ${
      ...
      "Ensure IApex is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested IApex base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.IApexApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm IApexai allowed-hostname <host>
      - then restart IApex and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@IApex.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "IApex", path: "/api/skills/IApex" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "IApex-create-agent",
          path: "/api/skills/IApex-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              IApexApiUrl: req.body.IApexApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.IApexApiUrl &&
          !persistedDefaults.IApexApiUrl
        ) {
          missingPersistedFields.push("IApexApiUrl");
        }
