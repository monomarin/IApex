const FAVICON_BLOCK_START = "<!-- IApex_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- IApex_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- IApex_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- IApex_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.IApex_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.IApex_WORKTREE_NAME) ?? nonEmpty(env.IApex_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.IApex_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="IApex-worktree-enabled" content="true" />',
    `<meta name="IApex-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="IApex-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="IApex-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
