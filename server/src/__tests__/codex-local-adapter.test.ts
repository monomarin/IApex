...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@iapexai/adapter-codex-local/server";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use IApex skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use IApex skill**" },
    ]);
  });
  ...
  it("parses command execution and file changes", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/IApexuser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
    ...
    ).toEqual([
      {
        kind: "system",
        ts,
        text: "file changes: update /Users/IApexuser/project/ui/src/pages/AgentDetail.tsx",
      },
    ]);
  });
