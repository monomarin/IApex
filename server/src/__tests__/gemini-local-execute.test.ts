...
import { execute } from "@iapexai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects IApex env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the IApex heartbeat.");
      expect(promptArg).toContain("IApex runtime note:");
      ...
      expect(invocationPrompt).toContain("IApex API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { IApex_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
