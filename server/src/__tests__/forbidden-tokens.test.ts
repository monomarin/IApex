...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "IApex", LOGNAME: "IApex", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "IApex" }),
      },
    );

    expect(tokens).toEqual(["IApex", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "IApex" }, {
        userInfo: () => ({ username: "IApex" }),
      });

      expect(tokens).toEqual(["IApex", "custom-token"]);
    } finally {
