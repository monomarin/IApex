...
describe("agent local JWT", () => {
  const secretEnv = "IApex_AGENT_JWT_SECRET";
  const ttlEnv = "IApex_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "IApex_AGENT_JWT_ISSUER";
  const audienceEnv = "IApex_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "IApex";
    process.env[audienceEnv] = "IApex-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
