...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@iapexai/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@iapexai/adapter-cursor-local/ui";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$IApex_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $IApex_API_KEY\"";
