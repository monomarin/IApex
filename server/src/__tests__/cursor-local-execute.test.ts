...
import { execute } from "@iapexai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects IApex env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the IApex heartbeat.");
      ...
      expect(capture.prompt).toContain("IApex runtime note:");
      expect(capture.prompt).toContain("IApex_API_KEY");
      expect(invocationPrompt).toContain("IApex runtime note:");
      expect(invocationPrompt).toContain("IApex_API_URL");
    } finally {
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const IApexDir = await createSkillDir(runtimeSkillsRoot, "IApex");
