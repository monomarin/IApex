...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("IApex OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested IApex base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("IApexApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.IApexApiUrl");
    expect(text).toContain("~/.openclaw/workspace/IApex-claimed-api-key.json");
    expect(text).toContain("IApex_API_KEY");
