...
import type { WorkspaceOperation } from "@iapexai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "IApex@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "IApex Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".IApex", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$IApex_WORKSPACE_BRANCH\" > .IApex-provision-branch",
        "printf '%s\\n' \"$IApex_WORKSPACE_BASE_CWD\" > .IApex-provision-base",
        "printf '%s\\n' \"$IApex_WORKSPACE_CREATED\" > .IApex-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".IApex-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".IApex-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".IApex-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".IApex-provision-created"), "utf8")).resolves.toBe("false\n");
  });
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-runtime-workspace-"));
  ...
  it("does not leak parent IApex instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "IApexConfig: process.env.IApex_CONFIG ?? null,",
          "IApexHome: process.env.IApex_HOME ?? null,",
          "IApexInstanceId: process.env.IApex_INSTANCE_ID ?? null,",
    ...
    process.env.IApex_CONFIG = "/tmp/base-IApex-config.json";
    process.env.IApex_HOME = "/tmp/base-IApex-home";
    process.env.IApex_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/IApex";
    ...
    expect(captured.IApexConfig).toBeNull();
    expect(captured.IApexHome).toBeNull();
    expect(captured.IApexInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "IApex-runtime-sibling-"));
