...
import { ensureCursorSkillsInjected } from "@iapexai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing IApex skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("IApex-cursor-skills-src-");
    const skillsHome = await makeTempDir("IApex-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "IApex");
    await createSkillDir(skillsDir, "IApex-create-agent");
    ...
    const injectedA = path.join(skillsHome, "IApex");
    const injectedB = path.join(skillsHome, "IApex-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "IApex")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "IApex-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "IApex"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "IApex-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("IApex-cursor-preserve-src-");
    const skillsHome = await makeTempDir("IApex-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "IApex");
    ...
    expect((await fs.lstat(path.join(skillsHome, "IApex-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("IApex-cursor-fail-src-");
    const skillsHome = await makeTempDir("IApex-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
