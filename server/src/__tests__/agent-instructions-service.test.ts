...
describe("agent instructions service", () => {
  const originalIApexHome = process.env.IApex_HOME;
  const originalIApexInstanceId = process.env.IApex_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalIApexHome === undefined) delete process.env.IApex_HOME;
    else process.env.IApex_HOME = originalIApexHome;
    if (originalIApexInstanceId === undefined) delete process.env.IApex_INSTANCE_ID;
    else process.env.IApex_INSTANCE_ID = originalIApexInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const IApexHome = await makeTempDir("IApex-agent-instructions-home-");
    const externalRoot = await makeTempDir("IApex-agent-instructions-external-");
    cleanupDirs.add(IApexHome);
    cleanupDirs.add(externalRoot);
    process.env.IApex_HOME = IApexHome;
    process.env.IApex_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("IApex-agent-instructions-new-external-");
    cleanupDirs.add(IApexHome);
    cleanupDirs.add(externalRoot);
    process.env.IApex_HOME = IApexHome;
    process.env.IApex_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("IApex-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
