...
import type { CompanyPortabilityFileEntry } from "@iapexai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "IApex",
        name: "IApex",
        description: "IApex coordination skill",
        markdown: "---\nname: IApex\ndescription: IApex coordination skill\n---\n\n# IApex\n",
        sourceType: "github",
        sourceLocator: "https://github.com/IApexai/IApex/tree/master/skills/IApex",
  ...
  it("exports referenced skills as stubs by default with sanitized IApex extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "IApex"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${IApexKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/IApexai/IApex/IApex/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/IApexai/IApex/IApex/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/IApexai/IApex/IApex/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".IApex.yaml"]);
    expect(extension).toContain('schema: "IApex/v1"');
    ...
    expect(extension).not.toContain("IApexSkillSync");
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/IApexai/IApex/IApex/SKILL.md"])).toContain("# IApex");
    expect(asTextFile(exported.files["skills/IApexai/IApex/IApex/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/IApexai/IApex/IApex/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/IApexai/IApex/IApex/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/IApexai/IApex/IApex/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .IApex.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".IApex.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-IApex",
        companyId: "company-1",
        key: "IApexai/IApex/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/IApexai/IApex/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "IApex_bundled",
          owner: "IApexai",
          repo: "IApex",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/IApexai/IApex/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/IApexai/IApex/release-changelog/SKILL.md"])).toContain("IApexai/IApex/release-changelog");
  });
  ...
  it("reads env inputs back from .IApex.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports a vendor-neutral package without .IApex.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported IApex");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".IApex.yaml"] = `${exported.files[".IApex.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
