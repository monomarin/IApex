...
function jwtConfig() {
  const secret = process.env.IApex_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.IApex_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.IApex_AGENT_JWT_ISSUER ?? "IApex",
    audience: process.env.IApex_AGENT_JWT_AUDIENCE ?? "IApex-api",
  };
}
