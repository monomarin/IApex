...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@iapexai/db";
import type { DeploymentMode } from "@iapexai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  IApexUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).IApexUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.IApexUpgradeContext = context;
