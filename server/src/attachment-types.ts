 ...
 * `IApex_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set.
 *
 * Examples:
 *   IApex_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   IApex_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.IApex_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.IApex_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
