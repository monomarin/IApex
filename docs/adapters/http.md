...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and IApex just triggers it.
...
1. IApex sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the IApex API
...
The external agent uses `IApex_API_URL` and an API key to call back to IApex.
