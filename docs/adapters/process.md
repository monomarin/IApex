...
- Running a Python script that calls the IApex API
...
1. IApex spawns the configured command as a child process
2. Standard IApex environment variables are injected (`IApex_AGENT_ID`, `IApex_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the IApex API and perform work.
