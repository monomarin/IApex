...
Build a custom adapter to connect IApex to any agent runtime.
...
2. Build environment with `buildIApexEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `IApexai run --watch` using `picocolors`.
...
Make IApex skills discoverable to your agent runtime without writing to the agent's working directory:
