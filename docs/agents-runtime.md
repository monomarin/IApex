...
Audience: Operators setting up and running agents in IApex

## 1. What this system does

Agents in IApex do not run continuously.  
...
For `claude_local` and `codex_local`, IApex assumes the CLI is already installed and authenticated on the host machine.
...
IApex stores session IDs for resumable adapters.
...
IApex pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. IApex surfaces this as a warning in environment tests, not a hard error.
