...
IApex supports two runtime modes with different security profiles.
...
```sh
pnpm IApexai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, IApex emits a one-time claim URL at startup:
...
```sh
IApex_DEPLOYMENT_MODE=authenticated pnpm IApexai run
```
