# ClipHub: Marketplace for IApex Team Configurations
...
> The "app store" for whole-company AI teams — pre-built IApex configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for IApex-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | IApex company (agents, projects, governance) |
...
A complete IApex company configuration:
...
Single-agent configurations designed to plug into a IApex org:
...
Portable skill files that any IApex agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with IApex's skill loading system
...
interface Listing {
  ...
  IApexVersionMin: string;     // Minimum IApex version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target IApex company
...
| `POST` | `/api/listings/:id/install` | Install to IApex company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target IApex company (or create new)
  → ClipHub API calls IApex API to:
  ...
  → Redirect to IApex dashboard with new team running
```
...
### 5.3 Creator: Export from IApex → Publish

```
Running IApex company → "Export as Blueprint" (CLI or UI)
  → IApex exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (IApex brand blue/purple)
...
| Install flow | One-click deploy to IApex company |
...
{
  "targetCompanyId": "uuid",         // Existing IApex company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `IApex-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as IApex
- **Backend**: Node.js API (or extend IApex server)
- **Database**: Postgres (can share IApex's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with IApex auth (or OAuth2)

### 9.2 Integration with IApex

ClipHub can be:
- **Option A**: A separate app that calls IApex's API to install blueprints
- **Option B**: A built-in section of the IApex UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing IApex UI and API.
...
- [ ] Install flow: blueprint → IApex company
