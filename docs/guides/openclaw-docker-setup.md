...
How to get OpenClaw running in a Docker container for local development and testing the IApex OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

IApex includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-IApex-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a IApex host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-IApex-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `IApex_HOST_PORT` (default `3100`)
- `IApex_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your IApex deployment is `authenticated`, provide auth context:

```bash
IApex_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
IApex_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host IApex server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed IApex URL (typically `http://host.docker.internal:3100`).
- If IApex rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm IApexai allowed-hostname host.docker.internal
```

Then restart IApex and rerun the smoke script.
