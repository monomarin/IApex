...
IApex exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`IApex_API_KEY`)
...
- Run audit trail: include `X-IApex-Run-Id` header on all mutating requests during heartbeats
