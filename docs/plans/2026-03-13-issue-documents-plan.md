...
Add first-class **documents** to IApex as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by IApex workflow/docs.
...
Update the IApex agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/IApex/SKILL.md`
