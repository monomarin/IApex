...
import { parseProjectMentionHref } from "@iapexai/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`IApex-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="IApex-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("IApex-mermaid-status", error && "IApex-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="IApex-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className="IApex-project-mention-chip"
  ...
  return (
    <div
      className={cn(
        "IApex-markdown prose prose-sm max-w-none break-words overflow-hidden",
