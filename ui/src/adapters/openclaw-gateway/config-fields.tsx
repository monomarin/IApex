...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="IApex API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "IApexApiUrl",
                  String(config.IApexApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "IApexApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://IApex.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "IApex"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. IApex persists a device key during onboarding so pairing approvals
