...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("IApex-worktree-enabled") !== "true") return null;

  const name = readMetaContent("IApex-worktree-name");
  const color = normalizeHexColor(readMetaContent("IApex-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("IApex-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
