...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("IApex-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("IApex-demo/COMPANY.md");
  ...
  it("reads a IApex zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".IApex.yaml": "schema: IApex/v1\n",
      },
      "IApex-demo",
    );
  ...
  it("reads standard DEFLATE zip archives created outside IApex", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("IApex-demo");
