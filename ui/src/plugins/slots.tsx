 ...
 *
 * Plugin bundles are built with `external: ["@iapexai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__IApexPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__IApexPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__IApexPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__IApexPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@iapexai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@iapexai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@iapexai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@iapexai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@iapexai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__IApexPluginBridge__) {
 ...
 * self-register via `window.IApexPlugins.registerReactComponent()`.
 ...
 * of `@iapexai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
