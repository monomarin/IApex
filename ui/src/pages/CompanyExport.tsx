...
/**
 * Filter .IApex.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterIApexYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://IApex.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm IApexai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [IApex](https://IApex.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [IApex](https://IApex.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .IApex.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .IApex.yaml
    const yamlPath = exportData.IApexExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterIApexYaml(exportData.files[yamlPath], checkedFiles);
    }
