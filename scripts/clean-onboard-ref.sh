...
Environment overrides:
  ...
  PC_DATA=/tmp/data           IApex data dir to use
  IApex_HOST=127.0.0.1    Host passed to the onboarded server
  IApex_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/IApex-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/IApex-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export IApex_HOST="${IApex_HOST:-127.0.0.1}"
export IApex_PORT="${IApex_PORT:-3100}"
export IApex_OPEN_ON_LISTEN="${IApex_OPEN_ON_LISTEN:-false}"
...
echo "IApex_HOST: $IApex_HOST"
echo "IApex_PORT: $IApex_PORT"
...
env \
  ...
  HOST="$IApex_HOST" \
  PORT="$IApex_PORT" \
  IApex_OPEN_ON_LISTEN="$IApex_OPEN_ON_LISTEN" \
  pnpm IApexai onboard --yes --data-dir "$PC_DATA"
