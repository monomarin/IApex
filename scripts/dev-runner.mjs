...
const devServerStatusFilePath = path.join(repoRoot, ".IApex", "dev-server-status.json");
...
const env = {
  ...process.env,
  IApex_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.IApex_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.IApex_MIGRATION_PROMPT ??= "never";
  env.IApex_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.IApex_DEPLOYMENT_MODE = "authenticated";
  env.IApex_DEPLOYMENT_EXPOSURE = "private";
  env.IApex_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[IApex] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[IApex] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@iapexai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[IApex] Command failed with code ${status.code}: pnpm --filter @iapexai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[IApex] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.IApex_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[IApex] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[IApex] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@iapexai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[IApex] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@iapexai/server", serverScript, ...forwardedArgs],
