#!/usr/bin/env bash
#
# Kill all local IApex dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all IApex dev processes
...
# Collect PIDs of node processes running from any IApex directory.
# Matches paths like /Users/*/IApex/... or /Users/*/IApex-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/IApex(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No IApex dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} IApex dev process(es):"
echo ""
