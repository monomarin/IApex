...
IMAGE_NAME="${IMAGE_NAME:-IApex-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
IApexAI_VERSION="${IApexAI_VERSION:-latest}"
...
IApex_DEPLOYMENT_MODE="${IApex_DEPLOYMENT_MODE:-authenticated}"
IApex_DEPLOYMENT_EXPOSURE="${IApex_DEPLOYMENT_EXPOSURE:-private}"
IApex_PUBLIC_URL="${IApex_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@IApex.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-IApex-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_IApexAI_VERSION=%q\n' "$IApexAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e IApex_DEPLOYMENT_MODE="$IApex_DEPLOYMENT_MODE" \
      -e IApex_DEPLOYMENT_EXPOSURE="$IApex_DEPLOYMENT_EXPOSURE" \
      -e IApex_PUBLIC_URL="$IApex_PUBLIC_URL" \
      -e IApex_HOME="/IApex" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "IApexai@${IApexAI_VERSION}" auth bootstrap-ceo --data-dir "$IApex_HOME" --base-url "$IApex_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$IApex_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$IApex_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$IApex_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$IApex_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$IApex_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$IApex_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg IApexAI_VERSION="$IApexAI_VERSION" \
...
echo "    Public URL: $IApex_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $IApex_DEPLOYMENT_MODE/$IApex_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/IApex-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$IApex_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $IApex_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$IApex_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $IApex_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
