...
IApex_HOST_PORT="${IApex_HOST_PORT:-3100}"
IApex_HOST_FROM_CONTAINER="${IApex_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.IApex-openclaw.override.yml"
...
detect_IApex_base_url() {
  ...
  for candidate in "$IApex_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${IApex_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${IApex_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
IApex_base_url="$(detect_IApex_base_url || true)"
...
IApex URL for OpenClaw container:
EOF
  if [[ -n "$IApex_base_url" ]]; then
    cat <<EOF
  $IApex_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${IApex_HOST_PORT}
  (Do not use http://127.0.0.1:${IApex_HOST_PORT} inside the container.)
  If IApex rejects the host, run on host machine:
    pnpm IApexai allowed-hostname host.docker.internal
  Then restart IApex and re-run this script.
EOF
  fi
  cat <<EOF
