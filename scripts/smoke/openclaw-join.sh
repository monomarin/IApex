...
COMPANY_ID="${COMPANY_ID:-${IApex_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-IApex-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-IApex-openclaw-smoke}"
...
Provide one of:
  IApex_AUTH_HEADER=\"Bearer <board-token>\"
  IApex_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "IApex OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.IApex.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
