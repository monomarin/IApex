...
function resolveIApexInstanceId(): string {
  const raw = process.env.IApex_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveIApexHomeDir(), "instances", resolveIApexInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveIApexHomeDir(), "instances", resolveIApexInstanceId(), "data", "backups");
}
