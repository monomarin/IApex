...
const IApex_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface IApexSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: IApexSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "IApex_required",
      originLabel: "Required by IApex",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by IApex",
    readOnly: false,
  };
}
...
export function buildIApexEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    IApex_AGENT_ID: agent.id,
    IApex_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.IApex_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.IApex_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.IApex_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.IApex_API_URL = apiUrl;
  return vars;
}
...
export async function resolveIApexSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...IApex_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listIApexSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<IApexSkillEntry[]> {
  const root = await resolveIApexSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the IApex skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "IApex cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredIApexRuntimeSkills(value: unknown): IApexSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: IApexSkillEntry[] = [];
...
export async function readIApexRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<IApexSkillEntry[]> {
  const configuredEntries = normalizeConfiguredIApexRuntimeSkills(config.IApexRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listIApexSkillEntries(moduleDir, additionalCandidates);
}

export async function readIApexSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listIApexSkillEntries(moduleDir);
...
export function readIApexSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.IApexSkillSync;
...
function canonicalizeDesiredIApexSkillReference(
  reference: string,
...
export function resolveIApexDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readIApexSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredIApexSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeIApexSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.IApexSkillSync;
  const current =
  ...
  next.IApexSkillSync = current;
  return next;
}

export async function ensureIApexSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the IApex server itself is started from
    // within a Claude Code session (e.g. `npx IApexai run` in a terminal
