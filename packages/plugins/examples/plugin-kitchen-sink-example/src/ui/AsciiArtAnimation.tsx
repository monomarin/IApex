...
type IApexSprite = (typeof IApex_SPRITES)[number];
...
function spriteSize(sprite: IApexSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = IApex_SPRITES[(baseCol + baseRow) % IApex_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = IApex_SPRITES[Math.floor(Math.random() * IApex_SPRITES.length)]!;
