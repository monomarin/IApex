...
Example IApex plugin that demonstrates:
...
```bash
pnpm --filter @iapexai/plugin-file-browser-example build
pnpm IApexai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm IApexai plugin uninstall IApex-file-browser-example --force
```
...
- Optional: use `IApex-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
