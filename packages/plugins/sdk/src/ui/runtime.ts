...
type GlobalBridge = typeof globalThis & {
  __IApexPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__IApexPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `IApex plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`IApex plugin UI component "${name}" is not callable`);
}
