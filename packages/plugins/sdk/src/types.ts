/**
 * Core types for the IApex plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@iapexai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @iapexai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of IApex object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the IApex UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit IApex domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core IApex domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the IApex secret provider
...
export interface PluginSecretsClient {
   ...
   * in the IApex secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@iapexai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@iapexai/plugin-sdk";
 *
