 ...
 * channel.
 *
 * The IApex plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@iapexai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all IApex messages.
 */
...
/**
 * IApex plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this IApex instance. */
    instanceId: string;
    /** Semver version of the running IApex host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
