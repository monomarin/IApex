/**
 * `@iapexai/plugin-sdk` — IApex plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@iapexai/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@iapexai/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @iapexai/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@iapexai/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@iapexai/plugin-sdk";
 *
