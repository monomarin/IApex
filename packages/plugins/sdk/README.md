...
Official TypeScript SDK for IApex plugin authors.

- **Worker SDK:** `@iapexai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@iapexai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@iapexai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@iapexai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@iapexai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@iapexai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@iapexai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@iapexai/plugin-sdk/ui/hooks` | Hooks only |
| `@iapexai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@iapexai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@iapexai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@iapexai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@iapexai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@iapexai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main IApex app. They can call ordinary IApex HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the IApex instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@iapexai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@iapexai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core IApex information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@iapexai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@iapexai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@iapexai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@iapexai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@iapexai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@iapexai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@iapexai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@iapexai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@iapexai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@iapexai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@iapexai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@iapexai/plugin-sdk/bundlers";
...
```bash
IApex-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@iapexai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__IApex__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__IApex__/events` streams `reload` SSE events on UI build changes
