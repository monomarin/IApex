...
/**
 * Zod schema for {@link IApexPluginManifestV1} — the complete runtime
 ...
 * | `minimumIApexVersion`| string?    | legacy alias of `minimumHostVersion`         |
 ...
 *
 * @see PLUGIN_SPEC.md §10.1 — Manifest shape
 * @see {@link IApexPluginManifestV1} — the inferred TypeScript type
 */
export const pluginManifestV1Schema = z.object({
  ...
  minimumIApexVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumIApexVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumIApexVersion
    && manifest.minimumHostVersion !== manifest.minimumIApexVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumIApexVersion must match when both are declared",
