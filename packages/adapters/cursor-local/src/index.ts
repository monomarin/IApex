...
Use when:
- You want IApex to run Cursor Agent CLI locally as the agent runtime
...
- IApex auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$IApex" and related skills on local runs.
- IApex auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
