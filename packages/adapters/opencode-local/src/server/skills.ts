...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside IApex management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readIApexRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureIApexSkillSymlink(available.source, target);
  }
