...
} from "@iapexai/adapter-utils";
import { asNumber, asString, buildIApexEnv, parseObject } from "@iapexai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "IApex";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "IApex";
  if (input.strategy === "run") return `IApex:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `IApex:issue:${input.issueId}`;
  return fallback;
}
...
function resolveIApexApiUrlOverride(value: unknown): string | null {
...
function buildIApexEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const IApexApiUrlOverride = resolveIApexApiUrlOverride(ctx.config.IApexApiUrl);
  const IApexEnv: Record<string, string> = {
    ...buildIApexEnv(ctx.agent),
    IApex_RUN_ID: ctx.runId,
  };

  if (IApexApiUrlOverride) {
    IApexEnv.IApex_API_URL = IApexApiUrlOverride;
  }
  if (wakePayload.taskId) IApexEnv.IApex_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) IApexEnv.IApex_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) IApexEnv.IApex_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) IApexEnv.IApex_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) IApexEnv.IApex_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    IApexEnv.IApex_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return IApexEnv;
}

function buildWakeText(payload: WakePayload, IApexEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/IApex-claimed-api-key.json";
  const orderedKeys = [
    "IApex_RUN_ID",
    "IApex_AGENT_ID",
    "IApex_COMPANY_ID",
    ...
    "IApex_TASK_ID",
    "IApex_WAKE_REASON",
    "IApex_WAKE_COMMENT_ID",
    "IApex_APPROVAL_ID",
    "IApex_APPROVAL_STATUS",
    "IApex_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = IApexEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = IApexEnv.IApex_API_URL ?? "<set IApex_API_URL>";

  const lines = [
    "IApex wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `IApex_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load IApex_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $IApex_API_KEY on every API call.",
    "- Use X-IApex-Run-Id: $IApex_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: IApex_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$IApex_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$IApex_COMPANY_ID/issues?assigneeAgentId=$IApex_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardIApexPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  IApexEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateIApex = parseObject(payloadTemplate.IApex);
  const workspace = asRecord(ctx.context.IApexWorkspace);
  const workspaces = Array.isArray(ctx.context.IApexWorkspaces)
    ? ctx.context.IApexWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.IApexRuntimeServiceIntents)
    ? ctx.context.IApexRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardIApex: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: IApexEnv.IApex_API_URL ?? null,
  };

  if (workspace) {
    standardIApex.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardIApex.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardIApex.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "IApex-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const IApexEnv = buildIApexEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, IApexEnv);
  ...
  const IApexPayload = buildStandardIApexPayload(ctx, wakePayload, IApexEnv, payloadTemplate);
