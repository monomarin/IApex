...
Use when:
- You want IApex to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the IApex server.
...
- workspaceRuntime (object, optional): desired runtime service intents; IApex forwards these in a standardized IApex.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- IApexApiUrl (string, optional): absolute IApex base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default IApex)

Standard outbound payload additions:
- IApex (object): standardized IApex context added to every gateway agent request
- IApex.workspace (object, optional): resolved execution workspace for this run
- IApex.workspaces (array, optional): additional workspace hints IApex exposed to the run
- IApex.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
