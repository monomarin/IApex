...
## Scope
This plan is now **gateway-only**. IApex supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses IApex skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a IApex task.
...
```bash
pnpm --filter @iapexai/server test:run -- openclaw-gateway
pnpm --filter @iapexai/server typecheck
pnpm --filter @iapexai/ui typecheck
pnpm --filter IApexai typecheck
```
