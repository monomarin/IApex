...
import type { AdapterModel } from "@iapexai/adapter-utils";
import { asString, runChildProcess } from "@iapexai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.IApex_PI_COMMAND === "string" &&
    process.env.IApex_PI_COMMAND.trim().length > 0
      ? process.env.IApex_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["IApex_", "npm_", "NPM_"] as const;
