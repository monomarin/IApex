...
Use when:
- You want IApex to run Pi (the AI coding agent) locally as the agent runtime
...
- IApex requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/IApexs/ and resumed with --session.
