/**
 * esbuild configuration for building the IApexai CLI for npm.
 *
 * Bundles all workspace packages (@iapexai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @iapexai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@iapexai/")) {
      externals.add(name);
    }
  }
