...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.IApex_API_URL;
    delete process.env.IApex_API_KEY;
    delete process.env.IApex_COMPANY_ID;
  });
