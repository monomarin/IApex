...
describe("IApexApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new IApexApiClient({
    ...
    expect(headers["x-IApex-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new IApexApiClient({ apiBase: "http://localhost:3100" });
