...
function buildSourceConfig(): IApexConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("IApex-pr-432")).toBe(
      path.resolve(os.homedir(), "IApex-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("IApex/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://IApex.example", 3110)).toBe("https://IApex.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/IApex-feature",
      homeDir: "/tmp/IApex-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/IApex-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/IApex-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.IApex_HOME).toBe(path.resolve("/tmp/IApex-worktrees"));
    expect(env.IApex_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.IApex_IN_WORKTREE).toBe("true");
    expect(env.IApex_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.IApex_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export IApex_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-secrets-"));
    const originalInlineMasterKey = process.env.IApex_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.IApex_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.IApex_SECRETS_MASTER_KEY;
      delete process.env.IApex_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.IApex_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.IApex_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.IApex_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          IApex_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.IApex_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.IApex_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".IApex-worktrees"),
      });

      const envPath = path.join(repoRoot, ".IApex", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("IApex_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("IApex_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/IApex_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.IApex_AGENT_JWT_SECRET;
      } else {
        process.env.IApex_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });

  it("defaults the seed source config to the current repo-local IApex config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".IApex", "config.json");
    const originalCwd = process.cwd();
    const originalIApexConfig = process.env.IApex_CONFIG;

    try {
      fs.mkdirSync(path.dirname(localConfigPath), { recursive: true });
      fs.writeFileSync(localConfigPath, JSON.stringify(buildSourceConfig()), "utf8");
      delete process.env.IApex_CONFIG;
      process.chdir(repoRoot);

      expect(fs.realpathSync(resolveSourceConfigPath({}))).toBe(fs.realpathSync(localConfigPath));
    } finally {
      process.chdir(originalCwd);
      if (originalIApexConfig === undefined) {
        delete process.env.IApex_CONFIG;
      } else {
        process.env.IApex_CONFIG = originalIApexConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/IApex",
        targetRepoRoot: "/Users/example/IApex-pr-432",
        workspaceCwd: "/Users/example/IApex",
      }),
    ).toBe("/Users/example/IApex-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/IApex",
        targetRepoRoot: "/Users/example/IApex-pr-432",
        workspaceCwd: "/Users/example/IApex/packages/db",
      }),
    ).toBe("/Users/example/IApex-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "IApex-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "IApex-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("IApex-make-test", {
        seed: false,
        home: path.join(tempRoot, ".IApex-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".IApex", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".IApex", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
