...
import { IApexConfigSchema, type IApexConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".IApex", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveIApexInstanceId());
}
...
export function readConfig(configPath?: string): IApexConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = IApexConfigSchema.safeParse(migrated);
  if (!parsed.success) {
