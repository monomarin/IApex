...
const JWT_SECRET_ENV_KEY = "IApex_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# IApex environment variables",
    "# Generated by IApex CLI commands",
...
export function resolveIApexEnvFile(configPath?: string): string {
...
export function loadIApexEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeIApexEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readIApexEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeIApexEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeIApexEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readIApexEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeIApexEnvEntries(next, filePath);
  return next;
}
