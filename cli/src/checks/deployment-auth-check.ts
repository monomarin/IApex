...
export function deploymentAuthCheck(config: IApexConfig): CheckResult {
  ...
  if (mode === "local_trusted") {
    if (!isLoopbackHost(config.server.host)) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `IApexai configure --section server` and set host to 127.0.0.1",
      };
    }
    return {
      name: "Deployment/auth mode",
  ...
  if (!secret) {
    return {
      name: "Deployment/auth mode",
      status: "fail",
      message: "authenticated mode requires BETTER_AUTH_SECRET (or IApex_AGENT_JWT_SECRET)",
      canRepair: false,
      repairHint: "Set BETTER_AUTH_SECRET before starting IApex",
    };
  }
  ...
  if (auth.baseUrlMode === "explicit" && !auth.publicBaseUrl) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `IApexai configure --section server` and provide a base URL",
    };
  }
  ...
  if (exposure === "public") {
    if (auth.baseUrlMode !== "explicit" || !auth.publicBaseUrl) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `IApexai configure --section server` and select public exposure",
      };
    }
    try {
      ...
      return {
        ...
        canRepair: false,
        repairHint: "Run `IApexai configure --section server` and provide a valid URL",
      };
    }
  }
