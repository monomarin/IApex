...
const DEFAULT_AGENT_JWT_ISSUER = "IApex";
const DEFAULT_AGENT_JWT_AUDIENCE = "IApex-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveIApexInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveIApexInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" IApex env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: IApexConfig | null = null;
...
function collectDeploymentEnvRows(config: IApexConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.IApex_PUBLIC_URL ??
    process.env.IApex_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.IApex_PUBLIC_URL
      ? "env"
      : process.env.IApex_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from IApex_PUBLIC_URL when possible)",
    },
    {
      key: "IApex_AGENT_JWT_TTL_SECONDS",
      value: process.env.IApex_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.IApex_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "IApex_AGENT_JWT_ISSUER",
      value: process.env.IApex_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.IApex_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "IApex_AGENT_JWT_AUDIENCE",
      value: process.env.IApex_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.IApex_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "IApex_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.IApex_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "IApex_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.IApex_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "IApex_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.IApex_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.IApex_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.IApex_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.IApex_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.IApex_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.IApex_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.IApex_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "IApex_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.IApex_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.IApex_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "IApex_CONFIG",
      value: process.env.IApex_CONFIG ?? configPath,
      source: process.env.IApex_CONFIG ? "env" : "default",
      required: false,
