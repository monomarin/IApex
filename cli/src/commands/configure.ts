...
function defaultConfig(): IApexConfig {
...
export async function configure(opts: {
  ...
  printIApexCliBanner();
  p.intro(pc.bgCyan(pc.black(" IApex configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `IApexai onboard` first.");
    p.outro("");
    return;
  }

  let config: IApexConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because IApex_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
