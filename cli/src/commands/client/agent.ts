...
import type { Agent } from "@iapexai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveIApexSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export IApex_API_URL='${escaped(input.apiBase)}'`,
    `export IApex_COMPANY_ID='${escaped(input.companyId)}'`,
    `export IApex_AGENT_ID='${escaped(input.agentId)}'`,
    `export IApex_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local IApex skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing IApex skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveIApexSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local IApex skills directory. Expected ./skills in the repo checkout.",
              );
            }
