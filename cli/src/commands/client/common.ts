...
import { ApiRequestError, IApexApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to IApex config file")
    .option("-d, --data-dir <path>", "IApex data directory root (isolates state from ~/.IApex)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the IApex API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.IApex_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.IApex_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.IApex_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set IApex_COMPANY_ID, or set context profile companyId via `IApexai context set`.",
    );
  }

  const api = new IApexApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.IApex_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.IApex_SERVER_PORT || "");
