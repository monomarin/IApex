...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@iapexai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://IApex:IApex@127.0.0.1:${port}/IApex`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printIApexCliBanner();
  p.intro(pc.bgCyan(pc.black(" IApex db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveIApexInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "IApex";
