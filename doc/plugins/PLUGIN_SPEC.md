# IApex Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for IApex's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main IApex app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary IApex HTTP APIs directly.
...
IApex plugin design is based on the following assumptions:

1. IApex is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core IApex business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by IApex core.
...
2. Let plugins add major capabilities without editing IApex core.
...
4. Depend on project-local plugin folders such as `.IApex/plugins`.
...
The single IApex deployment an operator installs and controls.

### 5.2 Company

A first-class IApex business object inside the instance.
...
A trusted in-process extension loaded directly by IApex core.
...
An installable instance-wide extension package loaded through the IApex plugin runtime.
...
IApex has two extension classes.
...
IApex already has a concrete workspace model:
...
Plugins live under the IApex instance directory.

Suggested layout:

- `~/.IApex/instances/default/plugins/package.json`
- `~/.IApex/instances/default/plugins/node_modules/`
- `~/.IApex/instances/default/plugins/.cache/`
- `~/.IApex/instances/default/data/plugins/<plugin-id>/`
...
IApex should add CLI commands:

- `pnpm IApexai plugin list`
- `pnpm IApexai plugin install <package[@version]>`
- `pnpm IApexai plugin uninstall <plugin-id>`
- `pnpm IApexai plugin upgrade <plugin-id> [version]`
- `pnpm IApexai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@IApex/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface IApexPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that IApex agents can use during runs.
...
- IApex server starts one worker process per installed plugin
...
Receives one typed IApex domain event.
...
export function definePlugin(definition: PluginDefinition): IApexPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: IApexPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@iapexai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@iapexai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@iapexai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@iapexai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@iapexai/plugin-sdk/ui`
...
The `@iapexai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the IApex secret provider system rather than a plain text input
...
1. Core IApex data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing IApex objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual IApex product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if IApex later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing IApex secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm IApexai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the IApex server. This is a normative requirement, not optional.
...
### 27.1 `@iapexai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@iapexai/plugin-test-harness";
...
For developing a plugin against a running IApex instance:

- The operator installs the plugin from a local path: `pnpm IApexai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-iapex-plugin`) that scaffolds:

- `package.json` with correct `IApexPlugin` keys
...
- `@IApex/plugin-workspace-files`
- `@IApex/plugin-terminal`
- `@IApex/plugin-git`
- `@IApex/plugin-linear`
- `@IApex/plugin-github-issues`
- `@IApex/plugin-grafana`
- `@IApex/plugin-runtime-processes`
- `@IApex/plugin-stripe`
...
- `@iapexai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@iapexai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@iapexai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@iapexai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@iapexai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@iapexai/plugin-sdk/ui`
...
- `@iapexai/plugin-test-harness`
- `create-iapex-plugin` starter template
...
IApex should not implement a generic in-process hook bag modeled directly after local coding tools.

IApex should implement:
...
That is the complete target design for the IApex plugin system.
