...
IApex V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a IApex-shaped design.

Assumption for this document: IApex is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class IApex objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into IApex.

The main conclusion is:

- IApex should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- IApex should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- IApex should use multiple extension classes instead of one generic plugin bag:
...
If IApex does this well, the examples you listed become straightforward:
...
Relevant IApex files reviewed for current extension seams:
...
It is too dangerous for IApex core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for IApex — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
IApex should absolutely do this.
...
IApex should copy this discipline.
...
IApex should adopt this style for plugin actions, automations, and UI schemas.
...
IApex can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for IApex too.

## What IApex Should Not Copy Directly
...
IApex is a control plane for an operator-managed instance with company objects.
...
IApex is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

IApex should not auto-load arbitrary code from a workspace repo like `.IApex/plugins` or project directories.
...
For IApex, equivalent hooks into:
...
IApex should not allow plugins to silently replace:
...
For IApex it would be risky because it combines:
...
IApex should require an explicit operator install step.

## Why IApex Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | IApex |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted IApex instance |
...
That means IApex should borrow the good ideas from `opencode` but use a stricter architecture.

## IApex Already Has Useful Pre-Plugin Seams

IApex has several extension-like seams already:
...
This is good news.
IApex does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended IApex Plugin Model
...
IApex already has this pattern implicitly:
...
- `plugin` means integration code that talks to IApex through a typed plugin protocol and capability model
...
- invoke explicit IApex actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@iapexai/plugin-sdk/ui`) with design tokens and common components.
...
IApex should treat plugin installation as a global instance-level action.

Examples:

- install `@IApex/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over IApex objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

IApex already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for IApex too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@IApex/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@iapexai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running IApex instance.

A `create-iapex-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how IApex achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the IApex server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. IApex's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@iapexai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@iapexai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For IApex

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@iapexai/plugin-sdk";

export default definePlugin({
  id: "@IApex/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit IApex entities
    });
    ...
    ctx.events.on("plugin.@IApex/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@iapexai/plugin-sdk/ui";
...
Any IApex plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific IApex objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@IApex/plugin-linear`)
...
- reuse core tables when the data is clearly part of IApex itself
...
If a concept is becoming part of IApex's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of IApex itself
...
### 3. Scope plugin data to IApex objects before adding custom schemas

A lot of plugin data naturally hangs off existing IApex objects:
...
If IApex eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@IApex/plugin-workspace-files`
...
Package idea: `@IApex/plugin-terminal`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving IApex.
- Board reviews the diff after a run without leaving IApex.
...
- optional `events.emit` (e.g. `plugin.@IApex/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@IApex/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@IApex/plugin-linear`

This plugin syncs IApex work with Linear. It is useful for:

- importing backlog from Linear
- linking IApex issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between IApex and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into IApex.
- IApex issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@IApex/plugin-github-issues`

This plugin syncs IApex issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to IApex issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with IApex task governance
...
- Plugin settings:
  ...
  - whether new IApex issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active IApex issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into IApex.
- GitHub webhooks update status/comment state in IApex.
- A PR is linked back to the IApex issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@IApex/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@IApex/plugin-grafana`

This plugin surfaces external metrics and dashboards inside IApex. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating IApex issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full IApex plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the IApex dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a IApex issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of IApex core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@IApex/plugin-runtime-processes`
...
Package idea: `@IApex/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into IApex. It is useful for:
...
- Failed payment spikes or churn events can generate IApex issues for follow-up.
...
- Stripe data should stay additive to IApex core
...
- plugin UI bundle loading, host bridge, `@iapexai/plugin-sdk/ui`
...
- `@iapexai/plugin-test-harness` and `create-iapex-plugin` starter template
...
IApex should not implement "an OpenCode-style generic in-process hook system."
IApex should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In IApex
...
8. Ship `@iapexai/plugin-test-harness` and `create-iapex-plugin` starter template.
