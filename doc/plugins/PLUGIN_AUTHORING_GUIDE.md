...
This guide describes the current, implemented way to create a IApex plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main IApex app.
...
```bash
pnpm --filter @iapexai/create-iapex-plugin build
node packages/plugins/create-iapex-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the IApex repo:

```bash
pnpm --filter @iapexai/create-iapex-plugin build
node packages/plugins/create-iapex-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/IApex/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@iapexai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@iapexai/plugin-sdk` from the local IApex checkout into a `.IApex-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into IApex from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@iapexai/plugin-sdk/ui`
