...
ClipHub is the public registry where people share, discover, and download IApex company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to IApex what a package registry is to a programming language. IApex already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in IApex — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own IApex instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the IApex spec. It contains:
...
**Install (fresh start):**
```
IApex install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local IApex instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within IApex, export your company as a template, then publish:

```
IApex export --template my-company
IApex publish cliphub my-company
```
...
ClipHub is a **separate service** from IApex itself. IApex is self-hosted; ClipHub is a hosted registry that IApex instances talk to.
...
| **IApex CLI** | `IApexai install`, `IApexai publish`, `IApexai cliphub sync` — built into IApex |
| **IApex UI** | "Browse ClipHub" panel in the IApex web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with IApex) |
| Backend | TypeScript + Hono (consistent with IApex) |
| Database | PostgreSQL |
...
4. Run `IApexai install cliphub:acme/lean-saas-shop`
5. IApex creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in IApex until it works well
2. Export: `IApexai export --template my-agency`
3. Publish: `IApexai publish cliphub my-agency`
...
3. Install just that agent: `IApexai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to IApex

ClipHub is **not required** to use IApex. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to IApex what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `IApexai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in IApex web UI
- [ ] `IApexai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with IApex's own philosophy)
