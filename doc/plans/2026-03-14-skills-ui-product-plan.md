...
This document defines the product and UI plan for skill management in IApex.
...
- IApex company import/export can include skills as package content
...
3. The UI must always tell the truth about what IApex knows:
   - desired state in IApex
...
IApex should treat skills at two scopes:
...
- created locally inside IApex later
...
7. “Understand what IApex knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a IApex-specific package layout
...
This matters because Codex and similar adapters may already have local skills that IApex did not install.

These should be clearly marked:

- external
- not managed by IApex
...
- `desired_only`: IApex wants it, adapter does not show it yet
- `external`: adapter has it but IApex does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, IApex only tracks desired state
...
- IApex can still track desired skills
...
- `.IApex.yaml` may add IApex-specific fidelity, but should not replace the base shortname association model
...
- `.IApex.yaml` may add IApex-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
IApex should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too IApex-specific.
...
That gives IApex one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
