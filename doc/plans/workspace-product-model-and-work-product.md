...
IApex needs to support two very different but equally valid ways of working:
...
Today, IApex already has the beginnings of this model:
...
- making IApex feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing IApex users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted IApex deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning IApex into a full code review product
...
This object must be recorded explicitly so that IApex can:
...
IApex should treat PRs as a type of work product linked back to:
...
IApex already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted IApex must be a first-class deployment mode

IApex cannot assume that it is running on the same machine as the code.

In cloud deployments, IApex may:
...
- `Runtime service`: a process or service IApex owns or tracks for a workspace
...
It also lets cloud-hosted IApex deployments point at codebases and remotes without pretending the IApex host has direct filesystem access.
...
Without an explicit `execution workspace` record, IApex has nowhere stable to attach:
...
IApex needs a single place to show:
...
- IApex-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without IApex owning the execution host
...
## 1. IApex host and execution host must be decoupled
...
without the IApex host itself running local git or local preview processes.
...
IApex should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for IApex.
...
IApex should keep workspace automation centered on a small set of hooks:
...
IApex should provide the equivalent visibility in the project `Code` surface:
...
IApex should treat execution workspaces as the natural home for:
...
IApex should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
IApex should continue treating cleanup policy as part of the core workspace model:
...
IApex should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the IApex host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

IApex should borrow Worktrunk's good execution patterns while keeping the broader IApex model:
...
That keeps IApex focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted IApex deployments remain first-class

That is a better fit for IApex than either extreme:
