...
IApex's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends IApex's API, UI, and data model | Yes |
...
modules/
  observability/
    IApex.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@iapexai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@iapexai/core";
...
```
1. Scan modules/ directory for IApex.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@iapexai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@iapexai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:IApex/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.IApex.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm IApexai store list                    # browse available modules and templates
pnpm IApexai store install <module-id>     # install a module
pnpm IApexai store import <template-id>    # import a company template
pnpm IApexai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one IApex instance | `server:started` |
...
4. **Module config** — `IApex.config.json` with per-module config, env var interpolation
...
10. **`@iapexai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
