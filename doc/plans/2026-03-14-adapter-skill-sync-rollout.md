...
This document defines the rollout plan for adapter-wide skill support in IApex.
...
- IApex stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

IApex currently has these adapters:
...
1. Desired skills live in IApex for every adapter.
...
These adapters have a stable local skills directory that IApex can read and manage.
...
These adapters do not have a meaningful IApex-owned persistent install state.
...
- desired skills stored in IApex
...
- runtime already injects IApex skills into `~/.cursor/skills`
...
- runtime already injects IApex skills into `~/.gemini/skills`
...
- runtime already injects IApex skills into `~/.pi/agent/skills`
...
- IApex can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects IApex skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- IApex may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed IApex skills that are clearly marked as IApex-managed
...
- IApex stores desired skills only
...
- avoid destructive wording unless IApex can prove a skill is IApex-managed
...
That gets IApex from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
