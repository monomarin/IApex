...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented IApex plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside IApex.
...
- package: `@iapexai/plugin-kitchen-sink-example`
- plugin id: `IApex.kitchen-sink-example` or `IApex-kitchen-sink-example`

Recommendation: use `IApex-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core IApex event such as `issue.created`
...
Recommendation: default to a IApex-local endpoint or a stable public echo endpoint to avoid flaky docs.
