...
This document defines the V1 design for a IApex adapter that integrates with
...
4. synthesized stdout events for IApex UI/CLI
...
3. **Agent callback auth to IApex** uses a bootstrap exchange flow (no long-lived IApex key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from IApex endpoints, not full SKILL.md prompt injection.
...
- `IApexPublicUrl` (optional override; else `IApex_PUBLIC_URL` env)
...
## IApex Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `IApex_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.IApexPublicUrl`
2. `process.env.IApex_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived IApex credentials in prompt text.

Flow:

1. Before launch/follow-up, IApex mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. IApex validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all IApex API calls.
...
1. Prompt includes a compact instruction to fetch skills from IApex.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/IApex`
   - `GET /api/skills/IApex-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <IApexPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active IApex run context.
...
5. `IApexPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from IApex skill endpoints |
| IApex auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from IApex context.
3. Add richer PR metadata surfacing in IApex UI.
