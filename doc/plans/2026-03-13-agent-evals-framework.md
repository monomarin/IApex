...
We need evals for the thing IApex actually ships:
...
- can we grow the suite over time from real IApex usage?
...
- Discussion #449: <https://github.com/IApexai/IApex/discussions/449>
...
IApex should take a **two-stage approach**:

1. **Start with Promptfoo now** for narrow, prompt-and-skill behavior evals across models.
2. **Grow toward a first-party, repo-local eval harness in TypeScript** for full IApex scenario evals.
...
3. The longer-term harness should run **real IApex scenarios** against seeded companies/issues/agents, not just raw prompt completions.
...
That is the right unit because that is what actually changes behavior in IApex.

## Why This Is The Right Shape

### 1. We need to evaluate system behavior, not only prompt output

Prompt-only tools are useful, but IApex’s real failure modes are often:
...
But OpenAI’s Evals API is not the right control plane for IApex as the primary system because our target is explicitly multi-model and multi-provider.
...
But IApex should still avoid making a hosted platform or a third-party config format the core abstraction before we have our own stable eval model.

The right move is:

- start with Promptfoo for quick wins
- keep the data portable and repo-owned
- build a thin first-party harness around IApex concepts as the system grows
...
These are the closest thing to prompt evals, but still framed in IApex terms.
...
The important part is that the case is about a IApex scenario, not a standalone prompt string.
...
IApex should grow eval coverage by promoting real runs into cases when we see:
...
- IApex needs scenario setup, control-plane state inspection, and multi-step traces as first-class concepts

Recommendation:

- use Promptfoo first
- store Promptfoo config and cases in-repo under `evals/promptfoo/`
- use custom JS/TS assertions and, if needed later, a custom provider that calls IApex scenario runners
- do not make Promptfoo YAML the only canonical IApex eval format once we outgrow prompt-level evals
...
- IApex must compare across models/providers
- we do not want our primary eval runner coupled to one model vendor

Recommendation:

- use the guidance
- do not use it as the core IApex eval runtime
...
- did they call the right IApex APIs?
...
Start with Promptfoo for immediate, narrow model-and-prompt comparisons, then grow into a first-party `evals/` framework in TypeScript that evaluates **IApex scenarios and bundles**, not just prompts.
...
But keep the canonical eval model inside the IApex repo and aligned to IApex’s actual control-plane behaviors.
