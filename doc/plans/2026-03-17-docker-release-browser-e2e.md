...
Today release smoke testing for published IApex packages is manual and shell-driven:
...
- runs `IApexai@${IApexAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@IApex.local`
- password: `IApex-smoke-password`
...
- the suite passes locally against both:
  - `IApexAI_VERSION=canary`
  - `IApexAI_VERSION=latest`
...
1. A published `IApexai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `IApexai@latest` Docker install can be smoke-tested by Playwright in CI.
