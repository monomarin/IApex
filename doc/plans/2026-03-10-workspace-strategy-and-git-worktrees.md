...
`PAP-447` asks how IApex should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same IApex instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into IApex:

- some operators use IApex to manage IApex and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as IApex
- Claude and Codex expose different built-in affordances, so IApex should not overfit to one tool

## Core Product Decision

IApex should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by IApex
...
- IApex should not assume that every adapter can see or use a host filesystem path directly
...
If IApex makes worktrees agent-first, it will blur:
...
- IApex core resolves and records execution workspace state
...
That gives IApex room to support:
...
- IApex handles workspace runtime services under the hood using project/workspace policy
...
Once IApex is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that IApex cannot infer
...
- when issue moves to done, should IApex auto-commit?
- should it auto-push?
...
IApex already has the right foundation for a project-first model.
...
For server-spawned local adapters, IApex already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that IApex should mirror directly

Implication:

- for `codex_local`, IApex should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but IApex should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, IApex should separate:
...
IApex should be able to express intentions such as:
...
These are not specific to IApex. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".IApex/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a IApex-specific `devServer` flag.
...
IApex should distinguish between:
...
IApex should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, IApex should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to IApex
...
For runtime services specifically, IApex should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as IApex-only dev-server flags
...
3. Inject resolved execution workspace details into `context.IApexWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / IApex instance can be reused through env/config injection when appropriate
...
- IApex can decide whether to reuse or start a fresh service deterministically
...
3. Test against IApex-on-IApex workflows first.
...
7. Workspace runtime services are modeled generically, not as IApex-specific dev-server toggles.
