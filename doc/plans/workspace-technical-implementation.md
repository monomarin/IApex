...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/IApex-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `IApex_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `IApex_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - IApex may create host-local worktrees, processes, and paths
- `cloud`
  - IApex should assume no durable host-local execution workspace management
...
- use `IApex_EXECUTION_TOPOLOGY` as a defaulting guardrail
