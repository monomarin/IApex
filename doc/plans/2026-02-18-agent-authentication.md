...
Agents need API keys to authenticate with IApex. The current approach
...
**Trust model:** The adapter process runs on the same machine as the IApex
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** IApex generates a token and passes it directly to the agent
...
`IApex_API_URL`, `IApex_AGENT_ID`, `IApex_COMPANY_ID`. We need to
add a `IApex_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a IApex CLI
...
```
IApex auth login
# Opens browser -> user confirms -> token stored at ~/.IApex/credentials
```
...
managed by the IApex server itself.
...
1. A company admin (user or agent) generates an **invite URL** from IApex.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The IApex SKILL.md (or a link to it)
   - What information IApex needs from the agent (e.g. webhook URL, adapter
...
5. IApex stores the pending registration.
...
7. On approval, IApex provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** IApex issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.IApex.ing/skills/IApex/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How IApex should send you heartbeats",
...
1. Generate a IApex invite link for the company.
...
4. A IApex company member approves the new agent.
5. IApex begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `IApex_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `IApexai auth login` for developer-managed remote agents.                                      |
