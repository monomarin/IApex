...
IApex already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model IApex should implement next.

## Product Goals

IApex should let operators:
...
For the next implementation pass, IApex should enforce these defaults:
...
IApex should separate subscription-included usage from billed spend:
...
IApex should have two threshold classes:
...
IApex must know that a project is budget-paused and enforce it.
...
IApex must also block execution before new work starts.
