...
- New config/env settings:
  - `IApex_AGENT_JWT_SECRET`
  - `IApex_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `IApex_AGENT_JWT_ISSUER` (default: `IApex`)
  - `IApex_AGENT_JWT_AUDIENCE` (default: `IApex-api`)
...
1. In:
   ...
   inject `IApex_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `IApex_API_KEY`, do not overwrite it.
- Continue injecting:
  - `IApex_AGENT_ID`
  - `IApex_COMPANY_ID`
  - `IApex_API_URL`
...
- Local adapters authenticate without manual `IApex_API_KEY` config.
