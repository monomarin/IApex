# IApex Memory Service Plan

## Goal

Define a IApex memory service and surface API that can sit above multiple memory backends, while preserving IApex's control-plane requirements:

- company scoping
- auditability
- provenance back to IApex work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current IApex architecture in:
...
IApex should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing IApex plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. IApex owns routing, provenance, and accounting

Providers should not decide how IApex entities map to governance.

IApex core should own:
...
The normalized IApex scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through IApex.

IApex should log every operation, whether the provider is local or external.
...
## What IApex Should Persist

IApex should not mirror the full provider memory corpus into Postgres unless the provider is a IApex-managed local provider.

IApex core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, IApex may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, IApex may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, IApex may capture selected issue comments or issue documents as memory sources.
...
IApex should give agents both automatic recall and explicit tools, with simple guidance:
...
IApex needs a first-class UI for memory, otherwise providers become black boxes.
...
This is important because a memory system that "works" but silently burns budget is not acceptable in IApex.
...
- it matches IApex's local-first posture
- it is inspectable
...
- Do we want IApex-managed extraction pipelines at all, or should built-ins be the only place where IApex owns extraction?
...
- IApex owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives IApex a stable "memory service" without locking the product to one memory philosophy or one vendor.
