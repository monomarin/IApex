...
Add a single storage subsystem for IApex that supports:
...
- API serves attachment bytes through authenticated IApex endpoints.
...
- Default local path is under instance root: `~/.IApex/instances/<instanceId>/data/storage`.
...
- `IApexai onboard` writes a valid `storage` config block by default.
- `IApexai configure --section storage` can switch between local and s3 modes.
...
- `IApexai doctor` reports actionable storage status.
