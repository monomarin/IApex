...
This document defines the next-stage plan for IApex company import/export.

The core shift is:

- move from a IApex-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside IApex.
...
IApex already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just IApex
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `IApex.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.IApex.yaml` sidecar for high-fidelity IApex-specific details
...
11. adapter-aware skill sync surfaces so IApex can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point IApex at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a IApex sidecar.
...
IApex must not redefine `SKILL.md`.
...
- IApex-specific fidelity lives in `.IApex.yaml`
- IApex may resolve and install `SKILL.md` packages, but it must not require a IApex-only skill format
...
### 5.4 Base Package Vs IApex Extension
...
- base package:
  ...
  - no IApex-only runtime fields by default
- IApex extension:
  - `.IApex.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by IApex tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`IApex.manifest.json` is not part of the future package direction.
...
IApex import/export should support these entity kinds:
...
- IApex can treat “this set of agents came from team package X” as the imported-team model
...
IApex should support source references in package metadata with:
...
IApex should define a new adapter surface area around skills:
...
Planned IApex behavior:

- if an adapter supports read, IApex should show current skills in the UI
- if an adapter supports write, IApex should let the user enable/disable imported skills
- if an adapter supports sync, IApex should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.IApex.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.IApex.yaml` alongside the base package
...
IApex should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `IApexai company export <company-id> --out <path>`
- `IApexai company import --from <path-or-url> --dry-run`
- `IApexai company import --from <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in IApex storage, or only referenced at import time?
...
3. Should IApex support direct local folder selection in the web UI, or keep that CLI-only initially?
