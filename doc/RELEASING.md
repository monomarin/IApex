...
Maintainer runbook for shipping IApex across npm, GitHub, and the website-facing changelog surface.
...
IApex uses calendar versions that still fit semver syntax:
...
2. **npm** — `IApexai` and public workspace packages are published
...
# or
npx IApexai@canary onboard --data-dir "$(mktemp -d /tmp/IApex-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/IApexai/IApex/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for IApex. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
IApexAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
IApexAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary IApexAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable IApexAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f IApex_version=canary
gh workflow run release-smoke.yml -f IApex_version=latest
```

Minimum checks:

- `npx IApexai@canary onboard` installs
