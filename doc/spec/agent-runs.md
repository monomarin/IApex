...
This spec defines how IApex actually runs agents while staying runtime-agnostic.
...
1. IApex is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, IApex manages lifecycle, and on exit IApex parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `IApex.skill` (shared IApex skill text block)
...
2. Preferred transport is env vars (`IApex_*`) injected at runtime.
